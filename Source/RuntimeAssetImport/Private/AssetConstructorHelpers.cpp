...
TArray<UMaterialInstanceDynamic *> GenerateMaterialInstances(UObject &Owner,
                                                             const TArray<FLoadedMaterialData> &MaterialDataList,
                                                             UMaterialInterface &ParentMaterialInterface)
{
    ...
    for (auto i = decltype(NumMaterials){0}; i < NumMaterials; ++i)
    {
        ...
        case EColorStatus::None:
            // if nothing is set
            UE_LOG(LogAssetConstructor, Error, TEXT("color status is not set in index %d"), i);

            break;
        case EColorStatus::ColorIsSet:
        {
            // log that no texture is found
            UE_LOG(LogAssetConstructor, Log, TEXT("No texture is found for material in index %d"), i);

            VerifyMaterialParameter(ParentMaterialInterface, EMaterialParameterType::Scalar,
                                    "TextureBlendIntensityForBaseColor");
            VerifyMaterialParameter(ParentMaterialInterface, EMaterialParameterType::Vector, "BaseColor4");

            // set to use color
            MaterialInstance->SetScalarParameterValue("TextureBlendIntensityForBaseColor", 0.0f);
        ...
        case EColorStatus::TextureIsSet:
        {
            ...
            // get texture
            UTexture2D *Texture0 = FImageUtils::ImportBufferAsTexture2D(CompressedTextureData);

            VerifyMaterialParameter(ParentMaterialInterface, EMaterialParameterType::Scalar,
                                    "TextureBlendIntensityForBaseColor");
            VerifyMaterialParameter(ParentMaterialInterface, EMaterialParameterType::Texture, "BaseColorTexture");

            // set to use texture
            MaterialInstance->SetScalarParameterValue("TextureBlendIntensityForBaseColor", 1.0f);
...
void VerifyMaterialParameter(const UMaterialInterface &MaterialInterface,
                             const EMaterialParameterType &MaterialParameterType, const FName &ParameterName)
{
    FMemoryImageMaterialParameterInfo MemoryImageMaterialParameterInfo(ParameterName);
    ...
    const auto &ParameterExists = MaterialInterface.GetParameterDefaultValue(
        MaterialParameterType, MemoryImageMaterialParameterInfo, MaterialParameterMetadata);

    // verify
    verifyf(ParameterExists, TEXT("Material %s doesn't have %s parameter."), *MaterialInterface.GetFName().ToString(),
