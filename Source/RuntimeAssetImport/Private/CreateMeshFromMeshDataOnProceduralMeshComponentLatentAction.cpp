...
FCreateMeshFromMeshDataOnProceduralMeshComponentLatentAction::
    ...
    : ExecutionFunction(InLatentInfo.ExecutionFunction), OutputLink(InLatentInfo.Linkage),
      CallbackTarget(InLatentInfo.CallbackTarget)
{
    ...
    auto MaterialInstances =
        GenerateMaterialInstances(InOutTargetProceduralMeshComponent, MaterialList, InOutParentMaterialInterface);
    ...
    for (auto Node_i = decltype(NumNodeList){0}; Node_i < NumNodeList; ++Node_i)
    {
        ...
        auto CalcTFTask = CalcTFMatrixTasks[Node_i] = Tasks::Launch(
            ...
            [ParentCalcTFTask, RelativeTransform]() mutable
            {
                ...
                const auto &TransformToTarget =
                    FTransform(RelativeTransform.ToMatrixWithScale() * ParentTransformToTarget.ToMatrixWithScale());
        ...
        for (auto Section_i = decltype(NumSections){0}; Section_i < NumSections; ++Section_i)
        {
            ...
            auto CalcVerticesRelativeToTargetTask = Tasks::Launch(
                ...
                [CalcTFTask, Vertices]() mutable
                {
                    ...
                    Algo::Transform(Vertices, VerticesRelativeToTarget, [&TransformToTarget](const FVector &vector)
            ...
            auto CalcNormalsRelativeToTargetTask = Tasks::Launch(
                ...
                [CalcTFTask, Normals]() mutable
                {
                    ...
                    Algo::Transform(Normals, NormalsRelativeToTarget, [&TransformToTargetRotator](const FVector &vector)
                ...
                [=, &InOutTargetProceduralMeshComponent]() mutable
                {
                    ExecuteOnGameThread(
                        ...
                        [=, &InOutTargetProceduralMeshComponent]() mutable
                        {
                            ...
                            const auto &VerticesRelativeToTarget = CalcVerticesRelativeToTargetTask.GetResult();

                            // get Normals relative to target
                            const auto &NormalsRelativeToTarget = CalcNormalsRelativeToTargetTask.GetResult();

                            // get Tangents relative to target
                            const auto &TangentsRelativeToTarget = CalcTangentsRelativeToTargetTask.GetResult();

                            // create mesh section
                            InOutTargetProceduralMeshComponent.CreateMeshSection_LinearColor(
                                MeshSectionIndex, VerticesRelativeToTarget, Section.Triangles, NormalsRelativeToTarget,
                                Section.UV0Channel, Section.VertexColors0, TangentsRelativeToTarget,
                                /* CreateCollision = */ true,
                                /* bSRGBConversion = */ false);
                        });
                },
                Tasks::Prerequisites(CalcVerticesRelativeToTargetTask, CalcNormalsRelativeToTargetTask,
            ...
            // set Material
            InOutTargetProceduralMeshComponent.SetMaterial(MeshSectionIndex, MaterialInstance);
...
void FCreateMeshFromMeshDataOnProceduralMeshComponentLatentAction::UpdateOperation(FLatentResponse &Response)
{
    Response.FinishAndTriggerIf(IsRunning == false, ExecutionFunction, OutputLink, CallbackTarget);
}

void FCreateMeshFromMeshDataOnProceduralMeshComponentLatentAction::Finish() { IsRunning = false; }
