                                                   ...
                                                   AActor *const Owner, const bool ShouldRegisterComponentToOwner)
{
    ...
    const auto &MaterialInstances = GenerateMaterialInstances(*Owner, MaterialList, *ParentMaterialInterface);
    ...
    for (auto Node_i = decltype(NumNodeList){0}; Node_i < NumNodeList; ++Node_i)
    {
        ...
        if constexpr (TypeTests::TAreTypesEqual_V<UProceduralMeshComponent, MeshComponentT>)
        {
            for (auto Section_i = decltype(NumSections){0}; Section_i < NumSections; ++Section_i)
            {
                ...
                // create mesh section
                MeshComponent->CreateMeshSection_LinearColor(Section_i, Section.Vertices, Section.Triangles,
        ...
        else
        {
            ...
            for (auto Section_i = decltype(NumSections){0}; Section_i < NumSections; ++Section_i)
            {
                ...
                SrcProcMeshComp->CreateMeshSection_LinearColor(
                    Section_i, Section.Vertices, Section.Triangles, Section.Normals, Section.UV0Channel,
                    Section.VertexColors0, Section.Tangents, CreateCollision, bSRGBCConversion);
            ...
            if constexpr (TypeTests::TAreTypesEqual_V<UStaticMeshComponent, MeshComponentT>)
            {
                // new StaticMesh
...
#if !WITH_EDITOR
                    BuildMeshDescriptionsParams.bFastBuild = true; // set fast build (mandatory in non-editor builds)
                    BuildMeshDescriptionsParams.bAllowCpuAccess = true;
#endif

                    // copy meshes from SrcProcMeshComp
                    StaticMesh->BuildFromMeshDescriptions({&ProceduralMeshDescription}, BuildMeshDescriptionsParams);
                }
            ...
            else if constexpr (TypeTests::TAreTypesEqual_V<UDynamicMeshComponent, MeshComponentT>)
            {
                ...
                MeshDescriptionToDynamicMesh.Convert(&ProceduralMeshDescription, DynamicMesh3, true);
            ...
            {
                // type check error
                static_assert([]() { return false; }(), "Only UProceduralMeshComponent, UStaticMeshComponent or "
        ...
        else
        {
            ...
            else
            {
                ...
                MeshComponent->AttachToComponent(ParentMeshComp, FAttachmentTransformRules::KeepRelativeTransform);
            }
        }
